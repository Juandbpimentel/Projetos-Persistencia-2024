...
async def buscar_empresas_com_page_e_limit(
    page: int, limit: int
) -> List[EmpresaDetalhadaDTO]:
...
def converte_ids_para_string(empresa: EmpresaDetalhadaDTO):
    ...
    for departamento in empresa["departamentos"]:
        ...
        departamento["funcionarios_id"] = [
            str(funcionario_id) for funcionario_id in departamento["funcionarios_id"]
        ]
...
async def trata_empresa_dict(empresa: Empresa):
    ...
    empresa_dict["departamentos_id"] = [
        ObjectId(departamento["_id"])
        for departamento in await db.departamentos.find(
            {"_id": {"$in": [ObjectId(d) for d in empresa_dict["departamentos_id"]]}}
        ).to_list()
    ]
    return empresa_dict
