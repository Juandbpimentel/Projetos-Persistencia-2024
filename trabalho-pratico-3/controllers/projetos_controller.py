...
async def buscar_projetos_com_page_e_limit(
    page: int, limit: int
...
async def trata_projetos_dict(projeto: Projeto) -> dict:
    ...
    projeto_dict["funcionarios_id"] = [
        ...
        for funcionario in projeto_dict["funcionarios_id"]
    ...
    projeto_dict["contrato_id"] = (
        ObjectId(projeto_dict["contrato_id"]) if projeto_dict["contrato_id"] else None
    )
    if projeto_dict["contrato_id"] and not await db.contratos.find_one(
        {"_id": projeto_dict["contrato_id"]}
    ):
...
async def create_projeto(projeto: Projeto) -> ProjetoDetalhadoDTO:
    ...
    for funcionario in projeto_criado["funcionarios"]:
        if not funcionario:
            raise HTTPException(
                status_code=404,
                detail=f"Funcionario {funcionario['_id']} não encontrado",
            )
...
async def update_projeto(projeto_id: str, projeto: Projeto) -> ProjetoDetalhadoDTO:
    ...
    await db_projetos.update_one({"_id": ObjectId(projeto_id)}, {"$set": projeto_dict})
    ...
    if (
        projeto_atualizado["contrato"] is None
        and projeto_antigo["contrato"] is not None
    ):
        await db.contratos.delete_one(
            {"_id": ObjectId(projeto_antigo["contrato"]["_id"])}
        )
